...
class Play:
    ...
    # no ko and suicide checking
    def check_boards_history(self, stone, boards):
        e = End()
        if 0 < len(boards) <= 3:
            if len(boards) == 3:
                board1 = boards[2]
            ...
            if len(boards) <= 2:
                ...
                if is_empty_board1 is True:
                    ...
                    # check for KO board 1 and board 3
                    is_ko = self.is_ko(board1, board3)
                    ...
                    if not is_ko and not double and not are_boards_diff_12 and not are_boards_diff_23 and turn:
                        return True
                    else:
                        return False
        else:
    ...
    def check_for_ko(self,stone,boards,row,col):
        if len(boards) != 3:
    ...
    def is_suicide(self,board,stone,row,col):
        ...
        opp_group_no_liberty = False
        if " " not in reached:
            while reached_coord:
                opp = reached_coord.pop(0)
                chain_opp, reached_opp, _ = board_obj.chain_and_reached(opp[0],opp[1])
                if " " not in reached_opp:
                    opp_group_no_liberty = True
                reached_coord = [x for x in reached_coord if x not in chain_opp]
            if opp_group_no_liberty is True:
                return False
            else:
                return True
        else:
...
if __name__ == '__main__':
    ...
    rules = RuleCheckerDriver()
    print(rules.driver(json_list))
